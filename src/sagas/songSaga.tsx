...
import { setSongs, addSong, updateSong, removeSong,setLoading, addSongSuccess, deleteSongSuccess, updateSongSuccess, clearSuccessMessage, setError, searchSongs, setPageNumber, setRowsPerPage, setSearchText, setTotal, setTotalSongs, setTotalArtists, setTotalAlbums, setTotalGenres, setAlbumCount, setGenreCounts, setAlbumCounts, setBackgroundcolor } from '../songSlice';
import { PayloadAction } from '@reduxjs/toolkit';


export function* fetchSongs(action: PayloadAction<{ pageIndex: number,pageSize: number; searchText: string, }>):Generator<any,void,any>{
    try{
        yield put(setLoading(true));
        const response = yield call(axios.get,`http://localhost:8800/api/songs?search_text=${action.payload.searchText}&page_size=${action.payload.pageSize}&page_index=${action.payload.pageIndex}`);
...
function* addSongSaga(action:ReturnType<typeof addSong>):Generator<any,void,any>{
    try{
        yield put(setLoading(true));
        const response = yield call(axios.post,'http://localhost:8800/api/songs',action.payload);
        if(response){
...
function* deleteSongSaga(action:ReturnType<typeof removeSong>):Generator<any,void,any>{

    try{
        yield put(setLoading(true));
        const songId=action.payload;

        const response = yield call(axios.delete,`http://localhost:8800/api/songs/${songId}`);
        if(response){
...
function* updateSongSaga(action:ReturnType<typeof updateSong>):Generator<any,void,any>{
    try{
        ...
        const response = yield call(axios.patch,`http://localhost:8800/api/songs/${id}`,action.payload);
        if(response){
...
function* clearSuccessMessageSaga() {
  ...
  function* overAllStatistics():Generator<any, void, any>  {
    yield put(setLoading(true))
    const response =yield call(axios.get, `http://localhost:8800/api/songs/totals`);
